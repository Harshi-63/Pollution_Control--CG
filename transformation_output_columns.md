...
| PM25_Filled | **R9b** | True if PM2.5 value was interpolated |
| SO2_Filled | **R9b** | True if SO2 value was interpolated |
| NOx_Filled | **R9b** | True if NOx value was interpolated |
...
## 15. `gap_analysis.csv` + `gap_fill_comparison.csv` — Rule T14

### `gap_analysis.csv` — Data Availability

| Column | Description |
|--------|-------------|
| Plant_ID, Stack_ID | Sensor identifier |
| Original_Readings | How many readings we actually received |
| OK/Faults/Maint/Unknown | Breakdown by status |
| Expected_Readings | How many we *should* have (time window / 15 min) |
| Missing_Readings | Expected - Original |
| Data_Availability_Pct | `Original / Expected x 100` |

### `gap_fill_comparison.csv` — Gap-Filled vs Raw Metrics

| Column | Description |
|--------|-------------|
| Pollutant | PM2.5 / SO2 / NOx |
| Values_Filled | Number of values interpolated by R9b |
| Fill_Rate_Pct | `Filled / Total x 100` |
| Raw_Mean | Mean of original (non-filled) values |
| Filled_Mean | Mean including interpolated values |
| Mean_Change_Pct | `(Filled - Raw) / Raw x 100` |
| Raw_Std, Filled_Std | Standard deviation comparison |
| Raw_Compliance_Pct | % readings within CPCB limit (raw only) |
| Filled_Compliance_Pct | % readings within limit (with fills) |
