...
# ═════════════════════════════════════════════════════════════
print("\n── T6: Compliance Rate ──")
compliance = df[df['Source_Type'] == 'Stack'].groupby('Plant_ID').agg(
...
gap_analysis = df.groupby(['Plant_ID','Stack_ID']).agg(
    Total=('Status','count'),
    Gap_Filled=('Status', lambda x: (x=='GAP_FILLED').sum()),
    Faults=('Status', lambda x: (x=='FAULT').sum())).reset_index()
gap_analysis['Availability_Pct'] = ((gap_analysis['Total']-gap_analysis['Gap_Filled'])/gap_analysis['Total']*100).round(2)
